...
constraints:
  ...
  - "every Signal Artifact (legacy Stage 1, SIG-*) has a non-empty `source` field; source_refs may be empty by category"
...
### Signal Artifact exception

Signal Artifacts (legacy Stage 1, IDs starting `SIG-`) are the pipeline's origin point
...
- **Empty source_refs.** Never, except for Signal Artifacts (`SIG-*` IDs),
...
After every 10 lineage audits OR on any Coherence Verification failure attributable to a
