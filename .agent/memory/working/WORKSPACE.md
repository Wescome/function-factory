...
2026-05-18T15:49:00Z: Production observability smoke `observability-smoke-1779118969` completed through SEED, CONTRACT, MAP, PATCH, VERIFY, and RELEASE. `/run-status/observability-smoke-1779118969` returned completed summary with 45 events and every execution node in `stepAccounting.ok`; `?logs=SEED` returned `runs/observability-smoke-1779118969/logs/SEED/attempt-1.log` with final `===STAGE_RESULT===`. Smoke exposed two reducer/attempt-accounting fixes now applied locally: queue redelivery attempt numbers are threaded into dispatcher messages, and terminal summary phase/terminalAt are preserved after post-terminal dispatcher events.
