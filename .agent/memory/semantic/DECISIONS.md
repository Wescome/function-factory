...
## 2026-04-19: Add EL- ArtifactId prefix + ExecutionLog schema (harness-bridge precondition)

**Decision:** Add `EL` to the `ArtifactId` prefix alternation in `packages/schemas/src/lineage.ts` and to `META_PREFIX_REGEX` in `packages/coverage-gates/src/checks.ts` (paired-PR lockstep, same discipline as CONTRACT and CTR). Add a new `ExecutionLog` Zod schema to `packages/schemas/src/execution-log.ts` extending Lineage with `workGraphId`, `adapterId`, `timestamp`, summary `status`, and `nodes` (default `[]`) per PRD-META-HARNESS-EXECUTE §"Schema additions required". Nested `ExecutionNodeRecord` carries `nodeId`, per-node `status`, optional `outcome`/`startedAt`/`completedAt`, and required `rationale`. Colocated tests verify prefix acceptance, regression of prior prefixes, schema structural validity, and that node/summary enums match the PRD (not Wes's original sketch).

**Rationale:** Precondition for harness-bridge core implementation (Stage 6 harness_execute Function). The CTR- entry above explicitly deferred EL- "to a subsequent paired PR landing alongside harness-bridge implementation, when ExecutionLog has a consumer" — that moment is now. The harness-bridge implementation must emit schema-conformant ExecutionLogs per PRD AC 4, so the schema must land first.

Divergences from Wes's original implementation sketch that were reconciled against PRD: (a) per-node status enum is `completed | failed | skipped | simulated | unknown` per AC 8, not `success | failure | timeout | skipped`; (b) summary status enum is `completed | failed | adapter_unavailable | partial` per AC 9 / line 156, not `success | failure | partial | uncomputable`; (c) `transitionHint` field described in the PRD is deferred until the upstream `FunctionLifecycleTransitionHint` schema is defined — present in PRD as "future iteration" (line 136), omitted here per YAGNI rather than added as an untyped permissive shape. Per Wes's stated reading rule: "where the PRD diverges from assumption, the PRD wins."

**Alternatives considered:** (a) Include a permissive `transitionHint: z.unknown().optional()` placeholder so the field exists but carries no shape. Rejected — a permissive field's validation does nothing; better to add the field in the paired PR where the `FunctionLifecycleTransitionHint` schema also lands. (b) Land EL- prefix without the ExecutionLog schema, saving schema work for the harness-bridge PR. Rejected — splits a paired-PR-shaped change, and the schema is what gives the prefix semantic meaning; prefix without consumer is exactly the anti-pattern the CTR- entry rejected.

**Status:** Proposed. Pending Architect approval.

## 2026-04-19: harness-bridge core + dry-run adapter landed; first ExecutionLog emitted

**Decision:** Implement `@factory/harness-bridge` package core per PRD-META-HARNESS-EXECUTE. New files: `src/types.ts` (HarnessAdapter, HarnessAdapterRegistry, AdapterNodeOutcome interfaces), `src/plan.ts` (derivePlan — deterministic alphabetical node sort), `src/execute.ts` (harnessExecute orchestrator with fail-closed adapter resolution, per-node try/catch, mechanical summary-status derivation, injectable clock), `src/dry-run-adapter.ts` (reference adapter — every node simulated), `src/emit.ts` (ExecutionLog YAML disk write, parallel to coverage-gates/emit.ts pure/IO split), `src/cli.ts` (thin wrapper loading a WorkGraph YAML file and invoking harnessExecute with a default registry containing dry-run). Colocated tests: 20 across 4 files covering happy path, adapter_unavailable, schema validation at boundary, summary-status derivation from heterogeneous per-node statuses, per-node failure from adapter throws, plan determinism, input immutability, and YAML emission round-trip.

**Rationale:** Closes the Stage 6 gap. Every Factory WorkGraph was previously operationally inert — no consumer existed. harness_execute with the dry-run adapter is the minimum viable Stage 6; it produces schema-conformant ExecutionLogs without any runtime dependency, satisfying every PRD acceptance criterion except the specific-adapter-implementation criteria (which are explicitly downstream per PRD lines 70/86/142). The plan/outcome split from PRD §Constraints (plan-deterministic, outcome-non-deterministic) is encoded directly: derivePlan is pure; the orchestrator accepts an injectable clock for deterministic testing but defaults to wall-clock for real invocations.

Ran harness_execute against WG-V2-CLASSIFY-COMMITS with the dry-run adapter. Emitted `specs/execution-logs/EL-WG-V2-CLASSIFY-COMMITS-2026-04-19T19-03-52-792Z.yaml` — 11 nodes simulated, summary status `completed`. This is the Factory's first operational telemetry artifact and the first ExecutionLog of any kind. Stage 6 is now end-to-end reachable. Stage 7 (TrustSignal derivation) and the assurance-graph have their input source.

Monorepo test count: 158 (59 schemas + 50 coverage-gates + 29 compiler + 20 harness-bridge). All typechecks clean across all packages.

**Alternatives considered:** (a) Build shell-exec adapter first because v2 is git-commit-triage (genuinely shell-invoking). Rejected — shell-exec opens a command-dispatch question the WorkGraph schema does not currently answer (see flag below); attempting it ad-hoc would either overfit to the v2 case or invent a schema field prematurely. Dry-run produces a valid ExecutionLog and unblocks Stage 7 design without resolving the dispatch question. (b) Skip the CLI. Rejected — the CLI is the driver that produces the first on-disk ExecutionLog; without it, the Factory has no demonstration that Stage 6 runs end-to-end against a real WorkGraph. (c) Make harnessExecute's registry a module-global mutable. Rejected — the registry is passed in explicitly to keep the function pure-dependency-injected, testable in isolation, and free of the hidden-global coupling that makes adapter-registration drift hard to audit.

**Outstanding flag (deliberately unresolved):** The shell-exec adapter was explicitly named in the implementation drop but is not included in this landing. Reason — the WorkGraph schema does not carry shell-command metadata on WorkGraphNode; nodes implement ArtifactId references (CONTRACT-*, INV-*, VAL-*) that represent specifications, not executable commands. For a shell-exec adapter to dispatch a node, the architecture must answer: where does the command string come from? Three candidate shapes (node-level schema extension, adapter-config lookup table, caller-provided dispatch function) each encode different ownership assumptions about per-node executable behavior. Wes flagged this earlier as "worth its own DECISIONS entry" and explicitly not for me to resolve ad-hoc. Shell-exec adapter awaits that decision.

**Status:** Proposed. Pending Architect approval.

## 2026-04-19: First ExecutionLog realized as physical artifact

**Observation:** `specs/execution-logs/EL-WG-V2-CLASSIFY-COMMITS-2026-04-19T19-03-52-792Z.yaml` exists on disk. The Factory has executed one of its own compiled WorkGraphs. Stage 6 has produced output. The whitepaper's "compile specifications to WorkGraphs, execute WorkGraphs to ExecutionLogs" claim is now realized through Stage 6, not merely stated.

**Claim:** Distinct from the harness-bridge-implementation Observed sibling above (which records the code landing), this entry records the artifact. Grep-able-on-disk evidence that the Factory reached architectural completeness through Stage 6 — a claim the bootstrap-stage-5-complete tag could not make, because its scope was Stage 5's WorkGraph output, not a WorkGraph's execution.
