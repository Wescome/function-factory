...
rationale: >
  The Factory compiles Executable Specifications (Stage 5 complete, multiple Executable Specifications on
  disk) but cannot produce code from them. Every Function remains at lifecycle
  state 'specified'. Fidelity Verification cannot evaluate without execution evidence. The
  closed loop (Stage 7 observability feeding Stage 1 signals) cannot activate
  because no Function has ever reached 'monitored'. This Intent Specification specifies the
  function-synthesis capability that transforms compiled Executable Specifications into
  ...
  The Intent Specification specifies all three as a single coordinated capability because
  ...
  (ArchitectureCandidate, Stage6TraceLog, RoleAdherenceReport, FidelityVerificationInput),
  and per-role Read/Write field lists from the prompt pack.
---

# Function Synthesis from Executable Specifications

## Problem

The Factory's Stage 5 compiler produces Executable Specifications from Intent Specifications. Nothing downstream transforms those specifications into running code, tests, configuration, and documentation. The pipeline is complete through specification but broken at production.

Every Function remains at lifecycle state 'specified' indefinitely. Fidelity Verification (acceptance review) cannot evaluate because it requires execution evidence that does not exist. Persistence Verification (assurance coverage) is unreachable because no Function has reached 'monitored'. The closed loop -- Stage 7 observability feeding runtime signals back into Stage 1 -- cannot activate because the loop has never completed a single traversal. The Factory is a specification engine, not yet a production system.

The pressure is maximal. PRS-META-FUNCTION-SYNTHESIS rates strength, urgency, and confidence all at 1.0. Every downstream Factory concern -- acceptance, assurance, trust composition, trajectory-driven closure -- is blocked on the absence of a function-synthesis capability. Whitepaper section 11 non-negotiable number 6 requires three fail-closed Verification Gates; Fidelity Verification cannot run without Stage 6 execution evidence. This is the forcing function.

The prior attempt to specify Stage 6 (IS-META-HARNESS-EXECUTE, retracted 2026-04-19) miscast the capability as generic node dispatch. The retraction established that Coherence Verification structural coverage does not imply conceptual correctness. This Intent Specification is authored under the bootstrap carve-out: the Architect fills the Critic role, performing semantic-alignment review against whitepaper section 3 before compilation.

## Goal

Deliver the function-synthesis capability: a five-role topology with pluggable binding modes that reads a compiled Executable Specification plus an ArchitectureCandidate and produces a complete Function implementation -- source code, tests, configuration, and documentation.

The capability is delivered through three co-specified Functions. The execution Function (FP-META-FUNCTION-SYNTHESIS-EXECUTION) orchestrates the five-role topology (Planner, Coder, Critic, Tester, Verifier), manages the decision algebra state, enforces repair-loop bounds, handles calibrated escalation at the Verifier, and emits produced code artifacts to disk. The control Function (FP-META-FUNCTION-SYNTHESIS-CONTROL) enforces role-adherence contracts, resolves disagreements between the Verifier verdict and acceptance review, and governs binding-mode selection. The evidence Function (FP-META-FUNCTION-SYNTHESIS-EVIDENCE) captures execution traces, candidate selection rationale, and the normalized acceptance evidence bundle that Fidelity Verification consumes.
...
Binding modes are the secondary deliverable. The binding-mode interface allows the same role contracts to execute via external harness delegation (Claude Code, Cursor, or equivalent) or via in-Factory role execution where each role is a Factory-managed agent. Additional binding modes are permitted but not required at v1. Specific binding-mode implementations are downstream Functions, not part of this Intent Specification.
...
Planner reads specEnvelope, executableSpecification, targetNodeIds, activeCandidate, repoContract, and validationOutcomes. Planner writes plan. Planner does not read or write code, does not execute tests, does not access tools beyond its read set. Planner's output is an execution plan that names which Executable Specification nodes to implement, in what order, with what constraints.

Coder reads plan, executableSpecification, activeCandidate, repoContract, editScopes, and repoContext. Coder writes patchProposals. Coder does not evaluate its own output, does not run tests, does not modify plan. Coder's output is bounded patch proposals against repository contracts.

Critic reads plan, patchProposals, executableSpecification, specEnvelope, and repoContract. Critic writes critique. Critic does not modify code, does not run tests, does not override the plan. Critic's output identifies defects, scope violations, missing validations, and invariant risks in the patch proposals.

Tester reads plan, patchProposals, critique, executableSpecification, scenarioManifest, and toolResults. Tester writes validationPlan and validationOutcomes. Tester does not modify code, does not modify the plan, does not override the critique. Tester selects and interprets validations, executing them via tool calls.
...
AcceptanceEvidence is a normalized projection conforming to the FidelityVerificationInput schema. Fidelity Verification never reads raw traces directly; it consumes the FidelityVerificationInput bundle, which includes artifact paths, validation outcomes, compile and test summaries, scope and constraint violation flags, repair loop count, resample summary, and provenance (harness_command, prompt_pack_version, tool_policy_hash, model_binding_hash, timestamps).
...
Produced code carries lineage back to the Executable Specification node that specified it. Every generated artifact path recorded in Stage6TraceLog.generated_artifact_paths traces to a specific Executable Specification node via the Planner's execution plan. The lineage chain is: Signal -> Pressure -> Capability -> FunctionProposal -> Intent Specification -> Executable Specification -> Executable SpecificationNode -> produced code file.
...
Successful execution triggers a crystallization check. If the execution path contains a novel pattern not already captured by an existing invariant or template, a new reusable artifact is proposed (not auto-committed -- it enters the Critic review flow). Crystallized artifacts enter specs/ with full lineage back to the execution that produced them. The crystallization check triggers after Persistence Verification passage, not as part of Persistence Verification itself. Crystallization check logic belongs in the runtime layer, not in the verification layer.
...
The evidence Function emits three artifacts per synthesis: ExecutionTrace (full Stage6TraceLog with resample tree), CandidateSelectionReport (weighted selection audit trail), and AcceptanceEvidence (normalized FidelityVerificationInput bundle). All three are emitted regardless of the terminal verdict. A synthesis that fails still produces evidence; the evidence shows why it failed.

## Acceptance criteria

### Execution Function (topology orchestration and code production)

1. Given a compiled Executable Specification and a selected ArchitectureCandidate, the execution Function instantiates the five-role topology under the candidate's specified binding mode and produces code files on disk. Test: provide a Executable Specification with at least three nodes; verify that code files appear at the expected paths and that each file traces to a Executable Specification node via the Planner's plan.
...
5. Given identical Executable Specification and ArchitectureCandidate inputs, the execution Function produces deterministic output modulo timestamps and non-deterministic model inference. Test: with a deterministic mock binding mode, run the same inputs twice and verify identical output artifacts.
...
9. No binding mode bypasses role-adherence checking. Both external delegation and in-Factory execution produce RoleAdherenceReports. Test: run the same Executable Specification through two different binding modes; verify both produce conforming RoleAdherenceReports.
...
11. Every synthesis produces an AcceptanceEvidence bundle conforming to the FidelityVerificationInput Zod schema. Fidelity Verification can evaluate the synthesis from the FidelityVerificationInput alone, without reading raw traces. Test: parse the emitted FidelityVerificationInput with FidelityVerificationInput.safeParse; verify it succeeds; verify all artifact paths resolve to files on disk; verify evidence.validation_outcomes is populated.
...
14. The binding-mode interface accepts an ArchitectureCandidate and a Executable Specification and returns produced artifacts plus a Stage6TraceLog. At least two binding modes are supported: external delegation and in-Factory execution. Test: instantiate each binding mode; verify both accept the same ArchitectureCandidate and Executable Specification inputs and both return conforming outputs.
...
A Executable Specification enters function synthesis and a complete Function exits with code on disk. Target: every Executable Specification that enters synthesis with a valid ArchitectureCandidate produces either code on disk (pass) or a complete evidence bundle explaining why not (fail). Zero silent failures where synthesis starts but produces neither code nor evidence.

Acceptance review receives normalized evidence from every synthesis. Target: 100% of syntheses produce a FidelityVerificationInput bundle that passes FidelityVerificationInput.safeParse. Fidelity Verification never needs to read raw traces.
...
The execution trace is reproducible given the same Executable Specification, ArchitectureCandidate, and deterministic binding mode. Target: byte-identical traces (modulo timestamps) on replay with deterministic mocks. Non-reproducibility is an execution Function defect triggering root-cause review.
...
Fidelity Verification evaluator implementation. Fidelity Verification consumes the FidelityVerificationInput bundle that this Intent Specification's evidence Function produces, but the evaluator logic that renders an acceptance verdict is a separate Intent Specification. This Intent Specification specifies what Fidelity Verification receives, not what Fidelity Verification does with it.

Persistence Verification continuous monitoring. Persistence Verification (assurance coverage) operates on deployed, monitored Functions. This Intent Specification produces Functions at lifecycle state 'implemented', not 'monitored'. Persistence Verification's continuous detector-freshness and evidence-source-liveness checks are a separate Intent Specification.

Specific binding-mode implementations. This Intent Specification specifies the binding-mode interface -- what a binding mode accepts (ArchitectureCandidate + Executable Specification) and what it returns (produced artifacts + Stage6TraceLog). The implementation of any specific binding mode (Claude Code delegation adapter, Cursor delegation adapter, in-Factory agent orchestrator) is a downstream Function specified in its own Intent Specification.

Second harness binding. Per DECISIONS 2026-04-24, the second binding-mode implementation is deferred until the first binding mode is proven in production. This Intent Specification requires the interface to support at least two modes; it does not require both to be implemented simultaneously.

Architecture search (candidate generation and selection). The ArchitectureCandidate consumed by function synthesis is selected upstream. The architecture-search capability that generates, evaluates, and selects candidates is a separate capability chain. This Intent Specification treats the ArchitectureCandidate as a provided input.
...
Cross-Executable Specification linking. A synthesis that references nodes from multiple Executable Specifications requires a cross-graph resolution mechanism not specified here. This Intent Specification's synthesis operates on a single self-contained Executable Specification per invocation.
