...
## Problem
The current Stage 5 compiler emits Executable Specification artifacts, but it does not emit ArchitectureCandidate artifacts that make the execution arrangement explicit before runtime exists.

## Goal
Implement a deterministic architecture-candidate execution engine that renders explicit candidate artifacts alongside Executable Specifications, capturing candidate execution arrangement without modifying compiler behavior in this bridge step.
...
Must keep ArchitectureCandidate artifacts separately addressable from Executable Specifications.
