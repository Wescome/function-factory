...
rationale: >
  Second meta-Intent Specification authored under Factory discipline. First non-Gate-1 Intent Specification,
  ...
  produces a Coherence Verification: PASS verdict on this Intent Specification, Coherence Verification's discipline is
  ...
  self-rewrite hooks on coherence-verification and prd-compiler skills are
  designed to consume.
  ...
  Function type; §6.4 for Persistence Verification's relationship to detector freshness
  (distinct from regression detection, which this Intent Specification specifies);
  ...
  invariant-authoring SKILL for detector well-formedness. This Intent Specification
...
This is the third control Function the Factory needs to be architecturally complete (Coherence Verification is the first, Persistence Verification — assurance coverage — is the second; regression detection is distinct from both). Unlike the Verification Gates, which run synchronously at compile time or at fixed lifecycle transitions, regression detection is continuous and stateful — it operates over time series, not over a single artifact. That shape is new to the Factory and is deliberately chosen as the first compiler stress test against non-Gate-1 shape.
...
detect_regression observes TrustSignal and emits FunctionLifecycle transitions. It does not compute TrustSignal values — that is the runtime package's responsibility per whitepaper §5. It does not observe incidents — that is the assurance-graph package's responsibility per §5. It does not observe detector freshness — that is Persistence Verification's concern per §6.4. A detect_regression implementation that read from the detector freshness store would be a Persistence Verification / regression-detection collapse and must be rejected at Critic Agent review.
...
Detector freshness. Whether the TrustSignal pipeline is live, whether named evidence sources are emitting, whether the audit path is intact — these are Persistence Verification's concerns per whitepaper §6.4. detect_regression assumes the pipeline is live; if it is not, the `uncomputable` DetectionReport status surfaces that assumption's failure but does not diagnose the root cause. Root-cause diagnosis is Persistence Verification's scope.

Policy authoring. The detection policy is an input to detect_regression, not its output. Authoring the policy is a separate task — potentially itself compiled from a Intent Specification under Factory discipline in a later iteration. For the first detect_regression implementation, a committed baseline policy is assumed.

## Shared ControlFunction shape

Regression detection, Fidelity Verification, and Persistence Verification are all control Functions per whitepaper §3.2. Recording the shape here avoids drift across their three Intent Specifications the same way the Coherence Verification Intent Specification recorded the shared GateEvaluator shape.
...
The shape applies here and will apply to Fidelity Verification's Intent Specification and Persistence Verification's Intent Specification without modification. Differences among control Functions are in the observation type, the policy type, the specific transitions emitted, and the report schema — not in the shape itself.

## Schema additions required

This Intent Specification requires two new schemas in `packages/schemas/src/`:
...
Two prefix additions to the ArtifactId regex in `lineage.ts`: `DR` for detection reports and `POL` for policies. Same paired-PR discipline as the recent CONTRACT addition — regex in lineage.ts and META_PREFIX_REGEX in verification must be updated in lockstep.

These are Class B architectural changes per ConOps §12.1 and require a DECISIONS.md entry before the implementation PR. The schema PR for this work is sequenced to land before the detect_regression implementation PR, same pattern as the Coherence Verification bootstrap.

## Downstream artifacts detect_regression will enable

A passing Coherence Verification verdict on this Intent Specification is the precondition for compiling detect_regression into a Executable Specification and implementing it. If the first compile fails, the failure diagnostic becomes the first non-self-referential divergence signal the Factory has emitted — which is the signal the self-rewrite hooks on coherence-verification and prd-compiler skills exist to consume. In that case the remediation sequence is: diagnose the failure (which check failed, which specific atoms or artifacts were named), determine whether the failure is a skill-derivation gap (Pass 3's template matching, Pass 0's section mapping) or a genuine specification gap in this Intent Specification, commit the remediation to the appropriate skill or Intent Specification, recompile. A failing first compile is not a setback; it is the Factory's first observation that Coherence Verification generalized imperfectly, and is more valuable than a passing compile that provided no such signal.

Whether the first compile passes or fails, the Verification Report (`VR-IS-META-DETECT-REGRESSION-COHERENCE-<timestamp>.yaml`) is the artifact of interest. That is the second Verification Report the Factory will have committed — the first one being Coherence Verification's own. Two Verification Reports in `specs/verification-reports/` is the smallest evidence base from which the Factory's audit claim can start to generalize.
