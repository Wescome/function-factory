...
Where the whitepaper makes an architectural claim, this ConOps translates that claim into operational rules. The whitepaper's six non-negotiables (lineage preservation, narrow-pass discipline, explicit invariants with detectors, assurance dependency typing, trajectory-driven closure with a birth gate, three Verification Gates fail-closed) are operationalized throughout this document as authority boundaries, mode rules, and scenario constraints. A change to this ConOps that violates any of the six is a change to the whitepaper, not a change to the ConOps alone.

## 2. Operational Context

### 2.1 The Factory in one paragraph

The Function Factory is a closed-loop compiler that converts Pressures (forcing functions on Koales.ai) into trustworthy executable Functions, and maintains the trust of those Functions over time through runtime evidence. It consists of seven stages (Signals → Pressures → Capabilities → Function Proposals → Intent Specification/Compiler → Agent Execution → Simulation and Runtime) and three Verification Gates (Compile, Simulation, Assurance). It produces Executable Specifications; it does not produce Work Orders. Executable Specifications are implementation artifacts. Work Orders are the organizational acts of commissioning that WeOps governs. The Factory is I-layer infrastructure. It is not a harness, not a code generator, and not WeOps.
...
The **Critic Agent** is a software role that reviews Coding Agent output against contracts, invariants, and past lessons. The Critic does not produce primary artifacts; it produces critiques that gate whether Coding Agent output advances to the next pipeline stage. The Critic reviews atom extractions for semantic drift, invariant specifications for detector completeness, Executable Specifications for lineage integrity, and Verification Reports for internal consistency.
...
The **Gate Evaluator** is not a human or an agent. It is the automated runner that executes the three Verification Gates against their specified inputs and emits Verification Reports. The Gate Evaluator has no discretion. It applies the coverage formulas in `packages/verification` to the Zod-validated inputs and produces a verdict: pass or fail, with the specific artifacts that failed coverage enumerated by ID.
...
The **Operator** is the human role responsible for monitoring Functions in the `monitored` lifecycle state, responding to Persistence Verification alerts, and coordinating incident response when assurance regressions occur. The Operator is not the Architect; the Operator is the daily shift. An Operator reviews the rolling Assurance Verification Reports, investigates detector freshness anomalies, confirms or rejects assurance-regression transitions, and escalates to the Incident Responder when a regression points to active behavioral failure.
...
| Gate verdict | Gate Evaluator | Coding Agent, Operator | Verification Report emission |
| Runtime alert | Gate Evaluator (continuous) | Operator | Persistence Verification transition event |
...
- Every artifact is tagged `META-` in its ID. Artifacts without this prefix are rejected at Coherence Verification during Bootstrap.
- Architect approval is required for every Verification Gate failure, not just high-impact ones. Bootstrap coverage is brittle by nature and false failures are more likely than in Steady-State; the Architect's review catches those before they become load-bearing lessons.
...
**Scope.** All seven stages active; all three Verification Gates active; trajectory-driven Function birth active with birth-gate scoring; dream cycle runs nightly; Persistence Verification monitors both META and non-META Functions continuously.
...
- Trajectory-driven proposals are generated and ranked; proposals above the birth-gate threshold enter Stage 5 as first-class Intent Specifications alongside human-authored Intent Specifications.
...
- A Verification Gate is offline (hardware, network, dependency). The Factory cannot compute verdicts.
...
- The offline component's gate is treated as **fail** for any artifact that would require its verdict. No silent passes. A Verification Gate that cannot compute its verdict does not produce a pass verdict by default; it produces a degraded-mode halt.
- Function lifecycle transitions that depend on the offline component are blocked. A Function cannot be promoted to `monitored` if Fidelity Verification is offline; it stays at `verified` until the gate is restored.
...
- A Persistence Verification failure pattern indicating systematic detector compromise (silent observers on live Functions).

**Rules specific to Emergency mode.**
- Verification Gates are not bypassed — they still run and still fail closed — but manual Architect-authorized overrides are permitted for specific, named artifacts with inline rationale in DECISIONS.md.
...
**Read authority** is universal across roles except for fields explicitly marked sensitive (e.g., raw signal sources that contain customer PII). Every role can read `specs/`, `.agent/memory/`, and Verification Reports. The exceptions are: the Auditor role cannot read in-flight Coding Agent session state (that's transient and unstable); the Coding Agent cannot read other Coding Agents' in-flight sessions (this enforces independence).

**Routine write authority** covers the everyday production of Factory artifacts: Coding Agents writing Pressures, Capabilities, Function Proposals, Intent Specifications; Gate Evaluator writing Verification Reports; all roles writing episodic memory entries. Routine writes do not require approval and are the default operational flow.
...
| Gate Evaluator | ✓ | Verification Reports only | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ |
...
Stage 5 is the **compiler**. Each Function Proposal produces a Intent Specification; each Intent Specification runs through the eight narrow passes; Coherence Verification evaluates the compiled output for coverage; on pass, a **Executable Specification** is emitted. The Executable Specification is the Factory's primary compiled artifact, the typed DAG that Stage 6 executes.

Stage 6 is **execution**. The Executable Specification is handed to a harness (Claude Code, Cursor, or a custom conductor) with a fixed node topology: Planner, Coder, Critic, Tester, Verifier. The execution produces code, tests, configuration, and documentation — the actual implementation artifacts of the Function.

Stage 7 is **simulation and runtime**. The implementation enters a Digital Twin Universe where scenarios exercise it against invariants; Fidelity Verification evaluates simulation coverage; on pass, the Function promotes from `verified` to `monitored`. Once `monitored`, the Function runs in production (or staging) with runtime telemetry feeding invariant detectors; Persistence Verification runs continuously; trust is computed from correctness, compliance, observability, stability, and user response.

Runtime evidence in Stage 7 closes the loop. **Trajectory** objects capture observed drift; **ProblemFrames** translate drift into problem statements; **FunctionProposals** are generated (with birth-gate scoring) for the drift; high-scoring proposals re-enter Stage 5 as new Intent Specifications. The Factory is therefore not a pipeline but a cycle.
...
### 6.3 Verification Report flow

Verification Reports are a distinct information channel that runs alongside the primary pipeline. Every Gate emits a Verification Report on every evaluation — pass or fail. Reports are written to `specs/verification-reports/` with the naming convention `VR-<artifact-id>-GATE<n>-<timestamp>.yaml`.

Verification Report flow is the primary channel for three downstream consumers: the Coding Agent (consuming Coherence Verification failures to remediate Intent Specifications), the Operator (consuming Fidelity Verification and Persistence Verification for lifecycle and monitoring decisions), and the Auditor (consuming all three for compliance review). During Bootstrap, the Architect is also a primary consumer of every Report, because every Bootstrap-phase Verification Report is an input to the next iteration of the Factory's own specification.

### 6.4 Incident flow

When an incident is declared — either manually by an Operator or Incident Responder, or automatically by Persistence Verification threshold breach — the incident flow activates. An **Incident** object is created with severity classification (sev1–sev4), the invariants it impacts, the Functions linked through those invariants, confidence, and status. The Incident enters the assurance dependency graph and propagates through the five dependency types (execution, evidence, policy, shared invariant, shared adapter), producing typed impact levels (watch, degraded, regressed) at every affected Function.
...
| Compiler invocation | per-Intent Specification | Coding Agent command |
| Coherence Verification evaluation | per-Intent Specification compilation | Pass 7 completion |
| Fidelity Verification evaluation | per-Function, pre-promotion | `verified` → `monitored` request |
| Persistence Verification evaluation | per-detector, continuous | detector freshness window |
...
4. The Coding Agent drafts a Intent Specification for the Function Proposal with the `IS-*` ID. The Intent Specification follows the template (problem, goal, constraints, acceptance criteria, success metrics, out-of-scope).
5. The Coding Agent invokes the compiler: `pnpm compile specs/intent-specifications/IS-XXX.md`. The compiler runs Passes 0–6, then Coherence Verification.
6. **If Coherence Verification passes**, the compiler emits the Executable Specification to `specs/executable-specifications/` and a Verification Report with `overall: pass` to `specs/verification-reports/`. The Coding Agent proceeds to Stage 6.
7. **If Coherence Verification fails**, the Verification Report with `overall: fail` is written and the compiler halts. The Coding Agent reads the Report, identifies the remediation scope (which atoms, invariants, or dependencies are missing), and revises the Intent Specification upstream. The compile is re-run.
8. With a passing Coherence Verification, the Coding Agent hands the Executable Specification to the harness. The Planner, Coder, Critic, Tester, Verifier topology executes against the Executable Specification. The Function enters `implemented` lifecycle state.
9. Required validations run. On full pass, the Function enters `verified` lifecycle state.
10. The Function is deployed into the Digital Twin Universe. Scenarios execute. Fidelity Verification evaluates simulation coverage.
11. **If Fidelity Verification passes**, the Operator confirms the `verified` → `monitored` transition, and Persistence Verification activates continuous monitoring.
12. **If Fidelity Verification fails**, the Function remains at `verified`. The Coding Agent adds scenarios, negative tests, or fixes failing required validations per the Verification Report's remediation text.
13. Persistence Verification runs continuously. Trust is computed from the five dimensions. The Function operates in production.

**Success criteria.** A Function reaches `monitored` state with trust composite ≥ 0.85, survives 24 hours without Persistence Verification failure, and has no incidents linked to its invariants.

### 7.2 Scenario B — Coherence Verification failure and Intent Specification remediation

**Precondition.** A Coding Agent has invoked the compiler on a Intent Specification and Coherence Verification has returned `overall: fail`.

**Flow.**

1. The Coding Agent reads the Verification Report at `specs/verification-reports/VR-IS-XXX-COHERENCE-<timestamp>.yaml`.
...
5. **Missing detector for invariant.** This is the most common Coherence Verification failure. The Coding Agent authors a DetectorSpec with evidence sources, direct rules, regression policy, and incident tags. A detector that cannot name its evidence source is rejected again at Coherence Verification; the remediation is upstream — the evidence source must first be made available (new telemetry, new audit stream, etc.).
...
8. The Coding Agent re-runs the compiler. If Coherence Verification passes, the flow returns to Scenario A step 7. If Coherence Verification fails again with a new defect class, the cycle repeats; if the same defect class recurs, the `on_failure` hook flags the relevant skill for self-rewrite and the Architect is notified.
9. All remediation actions are logged to episodic memory with pain_score ≥ 6, importance ≥ 7, and lineage back to the Intent Specification and the Verification Report.
...
**Precondition.** A Function is in `monitored` state. Persistence Verification detects a direct-rule violation in a critical invariant, or trust composite drops below threshold.

**Flow.**

1. The Gate Evaluator emits a Persistence Verification Verification Report with `overall: fail` and the specific invariant, detector, and evidence referenced.
...
3. The Operator is notified. The Operator investigates: reads the Verification Report, reads recent episodic entries tagged with the Function ID, inspects the linked Incident if one exists, reviews the evidence source state.
...
7. Remediation takes one of three shapes:
   - **Patch in place.** A new Intent Specification is drafted or an existing one is amended. The flow returns to Scenario A from step 4.
   - **Roll back.** The prior passing Executable Specification and implementation are restored. The Function re-enters `verified` state; Fidelity Verification must pass again for promotion to `monitored`.
...
5. Candidates above the birth-gate threshold enter the Architect's review queue. Candidates below threshold are logged and retained for pattern detection but do not produce Intent Specifications.
6. The Architect reviews the queue on the weekly scheduled review or ad-hoc for high-severity trajectories. Approval converts the FunctionProposal into a Intent Specification draft.
7. From there, the flow merges with Scenario A from step 4. The trajectory-driven Intent Specification is treated identically to a human-authored Intent Specification by the compiler and all downstream stages.
...
**Precondition.** A Function is in `monitored` state. Persistence Verification detects that one or more detectors have gone silent past their freshness threshold, or an evidence source has stopped emitting.

**Flow.**

1. Persistence Verification emits a Verification Report with `overall: fail` and details on the specific detector(s) or evidence source(s) affected.
...
5. Once evidence flow is restored, the Function does not auto-return to `monitored`. It returns to `verified`, and Fidelity Verification must be re-run against the scenario corpus before promotion. The rationale is that the Function's behavior during the evidence-loss window is unverified; treating it as still-monitored would back-date trust to a period that had none.
6. If the evidence loss was prolonged or the evidence source has changed shape materially, the invariant's detector spec itself may require amendment. That is a Intent Specification amendment and flows through the normal compiler path.
...
3. The override takes effect for the specific named artifact or action only. It does not generalize. An override on Function FN-123's Fidelity Verification failure does not relax Fidelity Verification for other Functions.
...
3. The following operational rules activate:
   ...
   - Verification Gate failures no longer require Architect approval by default; only repeated or high-impact failures surface.
...
5. The first vertical Intent Specification is authored by the Coding Agent for the agreed Function family and enters Stage 5 as the first non-META Intent Specification. From here the flow is Scenario A with non-META artifact IDs.
...
**Pass-internal error.** A compiler pass produces a runtime error not anticipated by the schema. The compiler halts, writes the exception with full pass state to `.factory/intermediates/error-<timestamp>.json`, and returns exit code 1. The Coding Agent reads the state, reproduces the error locally, and files a `compiler` skill issue. No Executable Specification is emitted.

**UncertaintyEntry emission.** A pass cannot confidently produce an artifact. An UncertaintyEntry is written with `pass`, `source`, `reason`, and `suggested_resolution`. The compiler halts with exit code 20. The Architect or the Coding Agent resolves the uncertainty — either by amending the Intent Specification, amending the pass's extraction heuristic (with a DECISIONS.md entry), or by marking the source as out-of-scope.

**Infinite loop or timeout.** A pass exceeds its configured time budget (default 120 seconds per pass). The compiler halts with exit code 30. The pass is flagged for performance review; this is a skill-level issue, not a IS-level one.

### 8.2 Gate exceptions

**Coherence Verification fails repeatedly on the same defect class.** The `on_failure` hook flags the relevant skill (usually `invariant-authoring` or `prd-compiler`) for self-rewrite. The Architect reviews whether the skill's template or the Intent Specification author's convention is the root cause.

**Fidelity Verification fails with zero scenarios covering a branch.** The Coding Agent adds scenarios. If the branch is unreachable by design, the Executable Specification has a dead node, which is a Stage 5 defect — the Intent Specification returns to compilation.

**Persistence Verification fails on detector registration.** A new invariant's detector was specified but the detector was never instantiated at runtime. The Operator escalates to the Coding Agent to connect the detector; the Function is held at `verified` until Persistence Verification can run.
...
**Override abuse.** The override rate exceeds the MOE threshold. The memory-manager surfaces the pattern; the Architect reviews whether the Factory's gates need calibration, whether the Intent Specification quality needs to improve, or whether the override authority needs to be subdivided.
...
**Security compromise of a detector.** A detector is reporting false healthy judgments (compromised observer). This is the most severe Persistence Verification failure because it looks like success. The Architect or Incident Responder manually invalidates the detector, transitions the affected Function to `assurance_regressed`, and forces Persistence Verification to refuse the compromised signal. The compromise is treated as a sev1 until the detector can be reauthenticated.
...
The Factory produces Functions. WeOps governs Work Orders. A Work Order may, when executed, run against a Function whose implementation is a Executable Specification. This is the only operational coupling between the two systems, and it is deliberately narrow.
...
**Outbound interface.** The Factory emits its Verification Reports, lifecycle transitions, and dream cycle outputs to Cognifiq.ai's ingest. Cognifiq.ai consumes these as evidence of the Factory's production quality and organizational learning.

**Inbound interface.** Cognifiq.ai may produce insights about the Factory's own operation — for example, detecting that certain Capability categories systematically produce Coherence Verification failures in particular subpopulations of Coding Agents. These insights are signals to the Factory at Stage 1, typed `internal`, and may produce Pressures that result in Factory self-improvement.
...
**Outbound interface.** The Factory publishes Functions in `monitored` lifecycle state to a catalog that vertical brands can consume. The catalog includes trust composite, invariant health, Verification Report links, and deployment metadata.
...
Claude Code, Cursor, OpenHands, SWE-agent, and similar coding agent harnesses are the execution substrates for Stage 6. The Factory is harness-agnostic; any harness that can read the Executable Specification schema and run the node topology (Planner, Coder, Critic, Tester, Verifier) is a valid Stage 6 host.

**Outbound interface.** The Factory emits a Executable Specification plus a node prompt pack (per skill file) plus context (Intent Specification, relevant memory, tool schemas). The harness consumes these and executes.

**Inbound interface.** The harness emits execution traces — tool calls, generated code, test results, Critic findings. These flow back to the Factory as Stage 6 output and become input to Fidelity Verification.

**Non-interface.** The harness does not modify Factory artifacts. The harness does not make Gate decisions. A harness that attempts to self-report Fidelity Verification pass without the Factory's Gate Evaluator running is an invalid harness.
...
**Intent Specification compilation success rate.** The fraction of compiler invocations that reach Coherence Verification pass on the first run. A low rate indicates either that Intent Specifications are being authored without sufficient rigor or that the compiler passes are mis-tuned for the Intent Specification format in use.

**Coherence Verification pass rate by defect class.** The rate at which each of the four coverage checks (atom, invariant, validation, dependency) fails. A concentrated failure class is a diagnostic signal — if invariant coverage is the dominant failure mode, the `invariant-authoring` skill needs reinforcement; if dependency closure is dominant, the Stage 4 proposal process is probably missing integration Functions.

**Time from Intent Specification to `monitored`.** The elapsed time between Intent Specification authorship and the Function reaching `monitored` state. Long times indicate Stage 6 or 7 bottlenecks; very short times suggest insufficient validation rigor.
...
**Regression rate.** Number of Functions transitioning from `monitored` to `regressed` per week. A rising rate is a signal that either the underlying environment is changing faster than the Factory's detection keeps up, or that Fidelity Verification's simulation coverage is insufficient.
...
**Time from Trajectory to new Function `monitored`.** The full cycle time for trajectory-driven closure. Long times indicate bottlenecks in Architect review, Intent Specification authoring, or harness execution.
...
2. The full set of six META-Pressures (from the whitepaper's non-negotiables) have been translated into formal Pressure artifacts, compiled through to Executable Specifications, implemented, and reached `monitored` state.
3. All three Verification Gates have been exercised: Coherence Verification has passed at least three compilations, Fidelity Verification has promoted at least one Function, Persistence Verification has been continuously active for at least 7 days without silent failure.
...
5. The Architect has reviewed Bootstrap Verification Reports and is satisfied that the gates are detecting actual specification defects, not producing noise.
...
- The completion-criteria evidence (with links to Verification Reports, Function IDs, dream cycle commits).
...
- The expected timeline for the first vertical Intent Specification.
...
- Bootstrap rules still partially active (Architect reviews all Coherence Verification failures, trajectory-driven Function birth disabled).
...
**Class A — Non-negotiable changes.** Changes to the whitepaper's six non-negotiables, the Verification Gate formulas, the lineage primitives, or this ConOps's authority model. These require Architect authorship and produce a new whitepaper or ConOps version. Class A changes are rare and architecturally significant.
...
**Class C — Operational changes.** Changes to specs/, memory/ (outside seeded lessons), Verification Reports, episodic entries. These are routine and do not require approval beyond the role's normal authority.
...
Every artifact in `specs/`, every file in `.agent/memory/semantic/`, every Verification Report, and every implementation file in `packages/*/src/` is under git version control. Every commit carries an artifact-ID-prefixed message. The git log is therefore a full lineage trace of the Factory's operational history.

Audit is performed by reading git log against the Auditor's read scope. An auditor reconstructing a Function's history walks the lineage chain: Signal → Pressure → Capability → FunctionProposal → Intent Specification → Executable Specification → implementation commits → Verification Reports → runtime events. Every edge of the chain is a git commit with explicit rationale.
