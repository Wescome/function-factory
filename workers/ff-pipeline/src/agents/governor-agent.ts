  ...
  async persist(result: GovernanceCycleResult): Promise<{ written: number; errors: string[] }> {
    ...
    try {
      await this.db.save('orl_telemetry', {
        ...
        // verdict maps to assessment.operational_health for debug endpoint visibility
        verdict: result.assessment.operational_health,
        trend: result.assessment.trend,
    ...
    // Write escalation signals as high-priority signals or GitHub Issues
    for (const escalation of result.escalations) {
      ...
      if (escalation.escalation_target === 'github_issue' && this.env.GITHUB_TOKEN) {
        try {
          await fetch('https://api.github.com/repos/Wescome/function-factory/issues', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
              title: `[Governor] ${escalation.issue}`,
              body: `## Governor Escalation\n\n**Severity:** ${escalation.severity}\n\n**Recommended Action:** ${escalation.recommended_action}\n\n**Evidence:**\n${escalation.evidence.map(e => `- \`${e}\``).join('\n')}\n\n---\n_Auto-generated by GovernorAgent (cycle: ${result.cycle_id})_`,
              labels: ['governor-escalation'],
            }),
          }).catch(() => {}) // best-effort, never block pipeline
          written++
        } catch (err) {
          errors.push(`github_issue/${escalation.issue.slice(0, 40)}: ${err instanceof Error ? err.message : String(err)}`)
        }
      }
    }
