...
describe('merge-readiness pack', () => {
  ...
  it('fails closed when audit and PR outcome lineage disagree', () => {
    expect(() => buildMergeReadinessPack({
      ...
      prOutcomeSignal: makeOutcome({
        raw: {
          ...makeOutcome().raw,
          executableSpecificationId: 'ES-DIFFERENT',
        },
      }),
      createdAt: '2026-05-06T04:00:00Z',
  ...
  it('fails closed when canonical MRP evidence is missing', () => {
    ...
    try {
      ...
      expect((err as MissingCanonicalMRPEvidenceError).missing).toEqual([
        ...
        'auditability.intentSpecificationId',
  ...
  it('overlays canonical MRP evidence with a persisted Fidelity Verification report id', () => {
    ...
    expect(evidence.soundVerification?.fidelityVerificationReportId).toBe('VR-FN-MOTDWVR2-W7UN-FIDELITY-2026-05-07T21-33-20-000Z')
    expect(evidence.soundVerification?.fidelityVerificationReportId).toBe('VR-FN-MOTDWVR2-W7UN-FIDELITY-2026-05-07T21-33-20-000Z')
    expect(evidence.auditability?.fidelityVerificationReportId).toBe('VR-FN-MOTDWVR2-W7UN-FIDELITY-2026-05-07T21-33-20-000Z')
    expect(evidence.auditability?.fidelityVerificationReportId).toBe('VR-FN-MOTDWVR2-W7UN-FIDELITY-2026-05-07T21-33-20-000Z')
  })

  it('keeps the legacy Fidelity Verification MRP evidence helper as a compatibility alias', () => {
    const evidence = withFidelityVerificationReportEvidence(
      makeCanonicalEvidence(),
      'VR-FN-MOTDWVR2-W7UN-FIDELITY-2026-05-07T21-33-20-000Z',
    )
  ...
  it('derives canonical functionId from proposalId at Merge Readiness assembly', () => {
    const pack = buildMergeReadinessPack({
      audit: makeAudit({
    ...
    const canonical = toCanonicalMergeReadinessPack(pack, {
      ...
      soundVerification: {
        ...
        fidelityVerificationReportId: 'VR-ABCDE123-FIDELITY',
      },
      seHygiene: {
        mentorRuleCompliance: [
          {
      ...
      rationale: {
        approach: 'Materialize the runtime verification artifact generated by synthesis',
        tradeoffsConsidered: 'Keep proposal identity until Merge Readiness assembly',
        prDescription: 'Factory dogfood materialization',
      },
      auditability: {
        intentSpecificationId: 'IS-ABCDE123',
        semanticReviewId: 'SRR-ABCDE123',
        coherenceVerificationReportId: 'VR-ABCDE123-COHERENCE',
        fidelityVerificationReportId: 'VR-ABCDE123-FIDELITY',
        modelBindings: {
          planner: { provider: 'factory-runtime', model: 'observed' },
        },
  ...
  it('rejects canonical functionId evidence that conflicts with Merge Readiness derivation', () => {
    const pack = buildMergeReadinessPack({
      audit: makeAudit(),
      prOutcomeSignal: makeOutcome(),
      createdAt: '2026-05-06T04:00:00Z',
    ...
    }))).toThrow(/must match the Merge Readiness functionId derived from pack\.proposalId/)
  })

  it('fails closed when Merge Readiness cannot build runtime MRP from a non-FP proposal', () => {
    expect(() => buildMergeReadinessPack({
      audit: makeAudit({
