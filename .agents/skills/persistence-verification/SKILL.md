...
`specs/verification-reports/VR-<FN-ID>-PERSISTENCE-latest.yaml` with versioned
history in `VR-<FN-ID>-PERSISTENCE-history.jsonl`:

```yaml
id: VR-<FN-ID>-PERSISTENCE-<timestamp>
...
2. If a class of miss keeps slipping through, propose a tighter threshold
   or additional check.
3. Commit: `META: skill-update: persistence-verification, {one-line reason}`
