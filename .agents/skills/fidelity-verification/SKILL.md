...
Runs before monitored promotion. Fidelity Verification verifies that the Function's scenario
suite actually exercises its specification in practice, not just in isolation.

## Inputs

- Function ID (lifecycle state: `verified`)
- Executable Specification that implements it
- Scenario corpus from the Digital Twin Universe
- ValidationSpec[] from the Function's Intent Specification compilation
- Invariant[] from the Function's Intent Specification compilation
- Run results for every validation

## Three coverage checks

### 1. Scenario coverage
Every branch in the Function's Executable Specification has been exercised by at least one
...
`specs/verification-reports/VR-<FN-ID>-FIDELITY-<timestamp>.yaml`:

```yaml
id: VR-<FN-ID>-FIDELITY-<timestamp>
...
checks:
  scenario_coverage:
    ...
    branches_unexercised:
      - executable-specification_node: <NODE-ID>
        edge: <EDGE-ID>
...
3. Commit: `META: skill-update: fidelity-verification, {one-line reason}`
