...
import { intentSpecificationIdFromFunctionProposalId } from "./ids.js"

/**
 * Deterministic Intent Specification renderer for the current narrow self-bootstrapping bridge.
 *
 * Supported proposals:
...
export function renderPrdFromFunctionProposal(
  ...
  const intentSpecificationId = intentSpecificationIdFromFunctionProposalId(proposal.id)

  if (proposal.id === "FP-META-CAPABILITY-DELTA-ENGINE") {
    const title = "Capability Delta Execution Engine"
    const markdown = [
      "---",
      `id: ${intentSpecificationId}`,
      "source_refs:",
      ...sourceRefs.map((r) => `  - ${r}`),
      ...
      "",
      "## Problem",
      "The repo can now compute capability delta and emit FunctionProposal demand during Function Proposal decomposition, but there is no dedicated execution engine artifact that computes capability delta as a first-class bootstrap Function from bounded repo evidence.",
      "",
      "## Goal",
      ...
      "",
      "Zero compiler behavior changes required to adopt the produced Intent Specification.",
      "",
      "## Out of Scope",
    ...
    return { id: intentSpecificationId, filename: `${intentSpecificationId}.md`, markdown }
  }
  ...
  if (proposal.id === "FP-META-SEMANTIC-REVIEW-EXECUTION") {
    ...
    const markdown = [
      ...
      "",
      "## Problem",
      "Coherence Verification verifies structural completeness, but the current repo has no semantic review execution step capable of blocking structurally valid yet conceptually invalid Intent Specifications before ExecutableSpecification emission.",
      "",
      "## Goal",
      "Implement a deterministic semantic review execution engine that consumes a IntentSpecification, CoherenceVerificationReport, and doctrine inputs, produces a semantic review verdict, and preserves fail-closed behavior before ExecutableSpecification emission without modifying compiler behavior in this step.",
      "",
      "## Constraints",
      "Must be fail-closed.",
      "",
      ...
      "5. The implementation remains separate from Intent-to-Executable compiler logic in this first bridge increment.",
      "",
      "## Success Metrics",
  ...
  if (proposal.id === "FP-META-ARCHITECTURE-CANDIDATE-EXECUTION") {
    ...
    const markdown = [
      ...
      "",
      "## Goal",
      "Implement a deterministic architecture-candidate execution engine that renders explicit candidate artifacts alongside ExecutableSpecifications, capturing candidate execution arrangement without modifying compiler behavior in this bridge step.",
      "",
      "## Constraints",
      ...
      "Must preserve lineage and explicit rationale in emitted artifacts.",
      "",
      "Must keep ArchitectureCandidate artifacts separately addressable from ExecutableSpecifications.",
      "",
      "Must not use LLM-based inference in the first implementation.",
      "",
      "## Acceptance Criteria",
  ...
  throw new Error(
    "Initial Intent Specification authoring bridge supports only FP-META-CAPABILITY-DELTA-ENGINE, FP-META-SEMANTIC-REVIEW-EXECUTION, and FP-META-ARCHITECTURE-CANDIDATE-EXECUTION"
  )
}
