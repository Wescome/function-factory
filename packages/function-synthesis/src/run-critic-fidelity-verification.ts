/**
 * Automated Critic review for IS-META-SIMULATION-COVERAGE.
 * First Intent Specification through universal Critic review — no carve-out.
 */
...
const prd = readFileSync(join(REPO, "specs/intent-specifications/IS-META-SIMULATION-COVERAGE.md"), "utf-8")
const wp = readFileSync(join(REPO, "specs/reference/The_Function_Factory_2026-04-18_v4.md"), "utf-8")
const s6 = wp.match(/## 6\. Spec Verification[\s\S]*?(?=## 7\.|$)/)?.[0]?.slice(0, 8000) ?? "§6 not found"

async function main() {
  console.log("=== AUTOMATED CRITIC REVIEW: IS-META-SIMULATION-COVERAGE ===\n")
  const model = getModel("anthropic", "claude-haiku-4-5-20251001")
  const stream = streamSimple(model, {
    systemPrompt: `You are the Critic. Review this Intent Specification for semantic alignment with whitepaper §6 (Spec Verification and Three Gates). Produce JSON only, no markdown fences: {"verdict":"aligned|miscast|uncertain","confidence":0.0-1.0,"citations":[{"section":"§6.X","quote":"...","supports":true,"reasoning":"..."}],"summary":"..."}`,
  ...
  try {
    ...
    const crvPath = join(REPO, `specs/critic-reviews/CRV-IS-META-SIMULATION-COVERAGE-${ts}.yaml`)
    writeFileSync(crvPath, `id: CRV-IS-META-SIMULATION-COVERAGE-${ts}
source_refs:
  - IS-META-SIMULATION-COVERAGE
