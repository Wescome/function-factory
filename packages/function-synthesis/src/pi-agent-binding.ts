...
export class PiAgentBindingMode implements BindingMode {
  ...
  async execute(
    ...
    await plannerAgent.prompt(
      `Produce a plan for ExecutableSpecification ${executableSpecification.id}. ` +
      `Nodes: ${executableSpecification.nodes.map((n) => n.id).join(", ")}. ` +
    ...
    await coderAgent.prompt(
      `Implement the plan for ExecutableSpecification ${executableSpecification.id}. ` +
    ...
    await criticAgent.prompt(
      `Review the plan and patches for ExecutableSpecification ${executableSpecification.id}. ` +
    ...
    await testerAgent.prompt(
      `Design and execute tests for ExecutableSpecification ${executableSpecification.id}. ` +
    ...
    await verifierAgent.prompt(
      `Review all artifacts for ExecutableSpecification ${executableSpecification.id}. ` +
    ...
    const patchProposals: PatchProposal[] = executableSpecification.nodes
      .filter((n) => n.type === "execution")
      .map((n) => ({
        targetPath: `src/${n.id}.ts`,
        content: `// Implementation for ${n.title}\nexport {}`,
        executableSpecificationNodeId: n.id,
