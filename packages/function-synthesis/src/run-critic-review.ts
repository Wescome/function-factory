/**
 * Step 7: Critic reviews a real Intent Specification for semantic alignment against whitepaper §3.
...
const intentSpecificationPath = join(REPO, "specs/intent-specifications/IS-META-COMPILER-PASS-8.md")
const prdContent = readFileSync(intentSpecificationPath, "utf-8")
...
console.log("=== CRITIC Intent Specification REVIEW (Step 7 — Bootstrap Carve-Out Expiration Test) ===\n")
console.log(`Intent Specification: IS-META-COMPILER-PASS-8`)
...
YOUR ROLE: Review this Intent Specification for SEMANTIC ALIGNMENT with whitepaper §3.

YOUR CONTRACT:
- You READ: the Intent Specification content, the whitepaper §3 text
- You WRITE: a typed review verdict
- You MUST NOT: modify the Intent Specification, modify the whitepaper, produce code

TASK: Compare the Intent Specification's conceptual model against whitepaper §3.
Produce a verdict:
- "aligned": the Intent Specification matches §3's definitions
- "miscast": the Intent Specification contradicts §3
...
async function main() {
  ...
  try {
    ...
    if (verdict.verdict && ["aligned", "miscast", "uncertain"].includes(verdict.verdict)) {
      ...
      console.log(`\n✅ Critic produced a real typed verdict with citations on a real Intent Specification.`)
