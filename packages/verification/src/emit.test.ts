...
function passingInput(): CoherenceVerificationInput {
  ...
  return {
    intentSpecificationId: "IS-META-EMIT-TEST",
    mode: "bootstrap",
    atoms: [atom],
    contracts: [contract],
    invariants: [inv],
...
describe("emitCoherenceVerificationReport", () => {
  ...
  it("writes a YAML file whose name matches <id>.yaml", async () => {
    ...
    const outDir = join(workDir, "specs", "verification-reports")
  ...
  it("produces YAML that roundtrips to an equivalent object", async () => {
    ...
    expect(roundtripped.intent_specification_id).toBe("IS-META-EMIT-TEST")
  ...
  it("YAML output contains key CoherenceVerificationReport fields", async () => {
    ...
    expect(contents).toContain("intent_specification_id: IS-META-EMIT-TEST")
