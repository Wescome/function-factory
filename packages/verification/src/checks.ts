...
type AtomVerificationResult = Checks["atom_coverage"]
type InvariantVerificationResult = Checks["invariant_coverage"]
type ValidationVerificationResult = Checks["validation_coverage"]
...
const META_PREFIX_REGEX =
  /^(PRS|BC|FN|CONTRACT|FP|IS|ES|INV|VAL|DEP|ATOM|VR|CTR|TRJ|PF|INC|DET|DEL|SIG|RGD|AC|ACS|RAD|EXS|EXT|EXR|EFF|EFFR|OBS|SNB|GOV|RPRS|DDI|CRL|SBI|PSR|GOVP|GOVD|GOVS|GOVA|GOVR|SRR|AMD|MR|CRP|VCR|MRP)-META-/
 ...
 */
export function checkAtomVerification(input: CoherenceVerificationInput): AtomVerificationResult {
 ...
 */
export function checkInvariantVerification(
 ...
 */
export function checkValidationVerification(
  input: CoherenceVerificationInput
 ...
 *
 * Walks the Intent Specification ID, every artifact's own ID, and every ID nested inside
...
export function checkBootstrapPrefix(
  ...
  check(input.intentSpecificationId)
