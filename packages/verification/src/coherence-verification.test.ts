...
describe("runCoherenceVerification", () => {
  ...
  it("populates source_refs with Intent Specification ID and every failing artifact ID", () => {
    const input = passingInput()
    const extra = makeAtom("ATOM-META-ORPHAN")
    const modified = { ...input, atoms: [...input.atoms, extra] } as CoherenceVerificationInput
    const report = runCoherenceVerification(modified, TIMESTAMP)
    expect(report.source_refs).toContain("IS-META-EXAMPLE")
  ...
  it("produces different ids for different timestamps, same content otherwise", () => {
    ...
    expect(reportA.intent_specification_id).toBe(reportB.intent_specification_id)
  })

  it("Verification Report ID starts with VR- and embeds the Intent Specification ID", () => {
    const report = runCoherenceVerification(passingInput(), TIMESTAMP)
    expect(report.id).toMatch(/^VR-IS-META-EXAMPLE-COHERENCE-/)
  })
  ...
  it("rationale is substantive and mentions Intent Specification ID and mode", () => {
    const report = runCoherenceVerification(passingInput("bootstrap"), TIMESTAMP)
    expect(report.rationale).toContain("IS-META-EXAMPLE")
