 ...
 *
 * Per IS-META-COHERENCE-VERIFICATION acceptance criterion 14- on pass,
  ...
  if (checks.atom_coverage.status === "fail") {
    ...
    sections.push(
      `Atom coverage- ${plural(ids.length, "orphan atom")} (${ids.join(", ")}). ` +
        ...
        `atom(s) to the Intent Specification's outOfScope list with rationale.`
    )
  }
    ...
    if (missingD.length > 0) {
      sections.push(
        `Invariant coverage (missing detector)- ${plural(
          ...
          `telemetry/audit layer, not at the Intent Specification layer.`
      )
    }
  }
  ...
  if (checks.bootstrap_prefix_check?.status === "fail") {
    ...
    sections.push(
      `Bootstrap prefix check- ${plural(ids.length, "non-META artifact ID")} ` +
        ...
        `ID with META- (e.g., ATOM-FOO → ATOM-META-FOO), or hold the Intent Specification ` +
