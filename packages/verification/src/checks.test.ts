...
function baseInput(overrides: Partial<CoherenceVerificationInput> = {}): CoherenceVerificationInput {
  return {
    intentSpecificationId: "IS-META-TEST",
    mode: "bootstrap",
    atoms: [],
    contracts: [],
    invariants: [],
...
describe("checkAtomVerification", () => {
  it("passes when every atom is referenced downstream", () => {
    ...
    const result = checkAtomVerification(input)
  ...
  it("sorts orphan_atoms deterministically", () => {
    ...
    const result = checkAtomVerification(baseInput({ atoms }))
    expect(result.orphan_atoms).toEqual([
      "ATOM-META-A",
      "ATOM-META-B",
      "ATOM-META-C",
...
describe("checkInvariantVerification", () => {
  it("passes when every invariant has validation and well-formed detector", () => {
    ...
    const result = checkInvariantVerification(input)
    expect(result.status).toBe("pass")
...
describe("checkValidationVerification", () => {
  it("passes when every validation has ≥1 covers* entry", () => {
    const val = makeValidation("VAL-META-V1", {
      coversAtomIds: ["ATOM-META-A"],
    })
    const result = checkValidationVerification(baseInput({ validations: [val] }))
    expect(result.status).toBe("pass")
  ...
  it("accepts coverage via any of the three covers* arrays", () => {
    ...
    const result = checkValidationVerification(
...
describe("checkBootstrapPrefix", () => {
  ...
  it("flags non-META IDs in source_refs", () => {
    const atom = makeAtom("ATOM-META-A", {
      source_refs: ["IS-VERTICAL-NOT-META"],
    })
    const input = baseInput({ atoms: [atom] })
    const result = checkBootstrapPrefix(input)
    expect(result.status).toBe("fail")
    expect(result.non_meta_artifact_ids).toContain("IS-VERTICAL-NOT-META")
  })
  ...
  it("flags non-META Intent Specification ID", () => {
    const input = baseInput({ intentSpecificationId: "IS-VERTICAL-EXAMPLE" } as CoherenceVerificationInput)
    const result = checkBootstrapPrefix(input)
    expect(result.status).toBe("fail")
    expect(result.non_meta_artifact_ids).toContain("IS-VERTICAL-EXAMPLE")
  })

  it("deduplicates and sorts non-META IDs", () => {
    const atomA = makeAtom("ATOM-META-A", {
      source_refs: ["IS-VERTICAL-Z"],
    })
    const atomB = makeAtom("ATOM-META-B", {
      source_refs: ["IS-VERTICAL-Z", "IS-VERTICAL-A"],
    })
