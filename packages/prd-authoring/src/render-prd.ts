...
export function renderPrdFromFunctionProposal(
  ...
  if (proposal.id === "FP-META-SEMANTIC-REVIEW-EXECUTION") {
    ...
    const markdown = [
      ...
      "",
      "## Problem",
      "Coherence Verification verifies structural completeness, but the current repo has no semantic review execution step capable of blocking structurally valid yet conceptually invalid PRDs before WorkGraph emission.",
      "",
      "## Goal",
      "Implement a deterministic semantic review execution engine that consumes a PRDDraft, CoherenceVerificationReport, and doctrine inputs, produces a semantic review verdict, and preserves fail-closed behavior before WorkGraph emission without modifying compiler behavior in this step.",
      "",
      "## Constraints",
      "Must be fail-closed.",
      "",
